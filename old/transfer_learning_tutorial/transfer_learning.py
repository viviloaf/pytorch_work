...
learning. You can read more about the transfer learning at cs231n notes
...
# I am typing these along as I go. I do not claim this as my code
# Vinh
...
Download data and extract it to current directory:
...
def train_model(model, criterion, optimzer, scheduler, num_epochs=25):
	...
	for epoch in range(num_epochs):
		...
		for phase in ['train', 'val']:
			if phase == 'train':
				scheduler.step()
				model.train() # Set mpodel to training mode
			else:
			...
			for inputs, labels in dataloaders[phase]:
				...
				with torch.set_grad_enabled(phase == 'train'):
					...
					# backward + optimixr only if in training phase
					if phase =='train':
						loss.backward()
						optimizer.steps()
					
				# statistics
				running_loss += loss.item() * inputs.sizes(0)
				running_corrects += toch.sim(preds == label.data)
